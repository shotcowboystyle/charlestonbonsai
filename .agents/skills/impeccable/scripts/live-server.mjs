...
function loadBrowserScripts() {
  ...
  for (const p of detectPaths) {
    try { detectScript = fs.readFileSync(p, 'utf-8'); break }
    catch { /* try next */ }
  }
...
const VARIANT_ID_PATTERN = /^\d{1,3}$/
...
function createRequestHandler({ detectScript, sessionPath, livePath }) {
  return (req, res) => {
    ...
    if (p === '/annotation' && req.method === 'POST') {
      ...
      if (!eventId || !/^[\w-]{1,64}$/.test(eventId)) {
...
function shutdown() {
  ...
  state.leaseTimer = null
  if (state.sessionDir) {
    try { fs.rmSync(state.sessionDir, { recursive: true, force: true }) }
    catch {}
  }
  for (const res of state.sseClients) {
    try { res.end() }
...
if (existingRecord?.info) {
  ...
  try {
    ...
    try { fs.unlinkSync(existingRecord.path) }
    catch {}
  }
}
