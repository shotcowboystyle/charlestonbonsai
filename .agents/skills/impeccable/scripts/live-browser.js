...
(function () {
  ...
  function showHighlight(el) {
    if (!el || !highlightEl)
      return
    const r = el.getBoundingClientRect()
    const top = `${r.top - 2}px`; const left = `${r.left - 2}px`
    const width = `${r.width + 4}px`; const height = `${r.height + 4}px`
  ...
  function onAnnotDown(e) {
    ...
    if (pinWrap) {
      ...
      try { annotOverlayEl.setPointerCapture(e.pointerId) }
      catch {}
      e.stopPropagation(); e.preventDefault()
      return
    }
  ...
  function onAnnotMove(e) {
    ...
    // kind === 'new'
    const dx = p.x - annotPointer.x0; const dy = p.y - annotPointer.y0
    if (!annotPointer.moved) {
      if (Math.hypot(dx, dy) < DRAG_THRESHOLD)
        return
      annotPointer.moved = true
  ...
  function onAnnotUp(e) {
    ...
    if (annotPointer.kind === 'pin') {
      ...
      try { annotOverlayEl.releasePointerCapture(e.pointerId) }
      catch {}
      annotPointer = null
      if (wasDrag) {
  ...
  function formatRangeValue(input) {
    const max = Number.parseFloat(input.max); const min = Number.parseFloat(input.min)
  ...
  function injectVariantsFromSource(filePath, sessionId) {
    ...
    fetch(url)
      .then((r) => {
        if (!r.ok)
          throw new Error(r.status); return r.text()
      })
      .then((html) => {
  ...
  function connectSSE() {
    ...
    evtSource.onmessage = (e) => {
      ...
      let msg; try { msg = JSON.parse(e.data) }
      catch { return }
      switch (msg.type) {
        case 'connected':
...
void main() {
  ...
  function teardown() {
    ...
    if (globalBarEl) {
      ...
      setTimeout(() => {
        if (globalBarEl)
          globalBarEl.remove(); globalBarEl = null
      }, 300)
    }
