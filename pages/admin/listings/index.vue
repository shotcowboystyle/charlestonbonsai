<template>
  <div>
    ...
    <div class="flex items-center justify-between mb-6">
      <div>
        <h1 class="font-serif text-2xl text-charcoal">Manage Listings</h1>
        <p class="text-stone-500 text-sm mt-1">{{ trees.length }} trees in collection</p>
      </div>
    ...
    <div class="bg-white rounded-xl shadow-soft overflow-hidden">
      ...
      <div v-else-if="filteredTrees.length === 0" class="p-12 text-center text-stone-500">
        <p v-if="search || filterStatus || filterType">No trees match your filters.</p>
        <p v-else>No trees yet. Add your first listing!</p>
      </div>

      <table v-else class="w-full">
        <thead class="bg-cream-50">
          <tr>
            <th class="px-6 py-3 text-left text-xs font-medium text-stone-500 uppercase tracking-wider">Tree</th>
            <th class="px-6 py-3 text-left text-xs font-medium text-stone-500 uppercase tracking-wider">Type</th>
            <th class="px-6 py-3 text-left text-xs font-medium text-stone-500 uppercase tracking-wider">Size</th>
            ...
            <th class="px-6 py-3 text-left text-xs font-medium text-stone-500 uppercase tracking-wider">Featured</th>
          ...
          <tr v-for="tree in filteredTrees" :key="tree.id" class="hover:bg-cream-50 transition-colors">
            <td class="px-6 py-4">
              <div class="flex items-center gap-3">
                ...
                <div>
                  <div class="font-medium text-charcoal">{{ tree.name }}</div>
                  <div class="text-xs text-stone-500">{{ tree.species }}</div>
                </div>
              </div>
            </td>
            <td class="px-6 py-4 text-sm text-stone-600 capitalize">{{ tree.treeType }}</td>
            <td class="px-6 py-4 text-sm text-stone-600">{{ TREE_SIZE_LABELS[tree.size] }}</td>
            <td class="px-6 py-4 text-sm font-medium text-charcoal">${{ tree.price.toLocaleString() }}</td>
            <td class="px-6 py-4">
...
import { TREE_TYPE_LABELS, TREE_SIZE_LABELS } from '~/types'
